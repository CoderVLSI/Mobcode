...
export function PetSprite({ pet, animation, size = 64 }: PetSpriteProps) {
  ...
  if (pet.stripPath) {
    const poseIndex = STRIP_POSES.indexOf(animation as any);
    const col = poseIndex >= 0 ? poseIndex : 0;
    const scale = s / STRIP_CELL_SIZE;
    const totalCols = STRIP_POSES.length;
    return (
      <Animated.View style={[
        ...
        <Image
          source={{ uri: `file://${pet.stripPath}` }}
          style={{
            width: STRIP_CELL_SIZE * totalCols * scale,
            height: STRIP_CELL_SIZE * scale,
            marginLeft: -col * STRIP_CELL_SIZE * scale,
          }}
          resizeMode="cover"
        />
      </Animated.View>
